...
if __name__ == '__main__':
    ...
    runs_to_plot = ['door_007_90deg_q5', 'door_008_90deg_q10','door_009_90deg_q25']
    # models_to_plot = model_log[(model_log['run name'].isin(runs_to_plot) ) & (model_log['ambient_temp'] == 5.0)]
    ...
    col_names = pd.MultiIndex.from_product([models_to_plot['quanta_gen_rate'].unique(),
                                            ...
                                            ['total', 'first room']],
                                    names=['quanta', 'gamma', 'wind_speed', 'amb_temp', 'wind_direction','group'])
    ...
    for i, file in enumerate(models_to_plot.index):
        ...
        df_plotting[(quanta, door, speed, temp,wind_dirx, 'total')] = model.get_risk_at_time(time_to_get_results)
        df_plotting[(quanta, door, speed, temp,wind_dirx, 'first room')] = model.get_risk_at_time(time_to_get_results,
                                                                                                 first_infection_group=True)
