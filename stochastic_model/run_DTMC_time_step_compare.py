...
simulation_constants = {'duration': timedelta(days=7),
                        'mask_efficiency': 0,
                        'lambda_home': 2000/898.2e4 / 24, # [hr^-1] should be quanta hr^-1? h
                        # 'lambda_home': 1, # [hr^-1] should be quanta hr^-1
                        'pulmonary_vent_rate': 0.54, # [m3.hr^-1]
                        'quanta_gen_rate': 10, # [quanta.hr^-1]
...
model.plot_SIR(ax=ax,ls='-', comparison='method', value='CTMC')

time_steps = [timedelta(minutes=60),
...
for time_step in time_steps:
    ...
    model.run(results_to_track=['S_df','I_df','R_df'])
    model.plot_SIR(ax=ax,ls='-', comparison=r'\$\Delta t\$', value=f'{time_step.minutes}mins')
